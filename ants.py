        ...
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                ...
                if place.ant.ant_contained is None:
                    ...
                    assert place.ant.ant_contained is None, "Too many ants in {0}".format(place)
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        dist = 0
        Taplace = self.place
        self.min_range = getattr(self, "lower_bound", self.min_range)
        self.max_range = getattr(self, "upper_bound", self.max_range)
        while Taplace.is_hive is False:
            if Taplace.bees:
                if self.min_range <= dist <= self.max_range:
                    return random_bee(Taplace.bees)
            Taplace = Taplace.entrance
            dist +=1
        return None
        # END Problem 3 and 4
    ...
    def action(self, gamestate):
        ...
        while curr_place != None:
            if curr_place.ant != None:
                ant = curr_place.ant
                if not hasattr(curr_place.ant, "damage_doubled") or not ant.damage_doubled:
                    ant.damage *= 2
                    ant.damage_doubled = True
                if ant.is_container and ant.ant_contained is not None:
                    contained_ant = ant.ant_contained
                    if not hasattr(contained_ant, "damage_doubled") or not contained_ant.damage_doubled:
